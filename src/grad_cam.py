...
class GradCam(torch.nn.Module):
    ...
    def forward(self, images):
        ...
        target_weight = torch.mean(self.backward_result, dim=(2, 3), keepdim=True)         # [N, C, 1, 1]
        # heatmap = torch.sum(a_k * self.forward_result, dim=0).detach().cpu().numpy()                  # [512, 7, 7] * [512, 1, 1]
        heatmaps = torch.mul(self.forward_result, target_weight)                      # [N, C, H, W]
        heatmaps = torch.sum(heatmaps, dim=1)                                         # [N, C, H, W]
...
def grad_cam(model_path, weight_path, image_size, train_loader, std, mean, device):
    # if torch.has_mps:
    ...
    cam = GradCam(model=model, image_size=image_size)
    heatmaps = cam(images)
